...
def extract_realistic_timestamps(transcript_text, duration_minutes, num_chapters=6):
    """Extract realistic timestamps from transcript or generate them based on duration"""
    try:
        ...
        if matches:
            # Convert to minutes and get evenly spaced timestamps
            all_timestamps = []
            for start, end in matches:
                start_minutes = float(start) / 60
                if start_minutes <= duration_minutes:
                    all_timestamps.append(start_minutes)

            if all_timestamps:
                # Select evenly spaced timestamps (ensure uniqueness)
                selected = []
                used_timestamps = set()
                for i in range(num_chapters):
                    target_time = (duration_minutes / num_chapters) * i
                    # Find closest unused timestamp
                    available = [t for t in all_timestamps if t not in used_timestamps]
                    if available:
                        closest = min(available, key=lambda x: abs(x - target_time))
                        selected.append(closest)
                        used_timestamps.add(closest)
                    else:
                        # Fallback to calculated time
                        selected.append(target_time)

                # Convert back to MM:SS format
                return [f"{int(t)}:{int((t % 1) * 60):02d}" for t in selected]

        # Fallback: generate evenly spaced timestamps
        timestamps = []
        for i in range(num_chapters):
            minutes = (duration_minutes / num_chapters) * i
            timestamps.append(f"{int(minutes)}:{int((minutes % 1) * 60):02d}")
        return timestamps
...
def extract_chapters(transcript_path, min_duration_minutes=45):
    ...
    # Extract realistic timestamps from the transcript
    realistic_timestamps = extract_realistic_timestamps(transcript_text, duration, 6)

    # Create a simple prompt that just asks for chapter titles using our realistic timestamps
    prompt = f"""Analyze this podcast transcript and create exactly 6 chapters using the provided timestamps.

Episode duration: {duration:.0f} minutes

Use EXACTLY these timestamps (do not change them):
{', '.join(realistic_timestamps)}

For each timestamp, provide a concise chapter title (3-6 words) based on the content around that time.

Format each chapter like this:
CHAPTER: MM:SS – Chapter Title Here

Example:
CHAPTER: 00:00 – Introduction and Setup
CHAPTER: 16:00 – Main Topic Discussion
CHAPTER: 32:00 – Deep Dive Analysis
    ...
    try:
        ...
        for line in lines:
            ...
            if line.startswith('CHAPTER: '):
                chapter_text = line[9:].strip()  # Remove 'CHAPTER: '
                if ' – ' in chapter_text:
                    timestamp, title = chapter_text.split(' – ', 1)
                    # Use timestamp as-is since we provided realistic ones
                    chapters.append({
                        'timestamp': timestamp.strip(),
                        'title': title.strip()
                    })
...
def format_chapters_for_summary(chapters, duration_minutes):
    ...
    formatted = f"\n## 📑 Episode Chapters *({duration_minutes:.0f} minutes)*\n\n"

    for chapter in chapters:
        formatted += f"- **{chapter['timestamp']}** – {chapter['title']}\n"
